...
namespace x
{
	...
	//static EGLNativeWindowType nativeWindow = nullptr;
	...
	x_Bool x_Graphics::create(x_Platform::x_WindowHandler *windowHandler, 
		                      x_Platform::x_DeviceContextHandler *deviceContextHandler)
	{
		...
		void* handler = x_Platform::GetHandler(m_deviceContextHandler);
		...
		EGLNativeWindowType win = (EGLNativeWindowType)x_Platform::GetHandler(windowHandler);
	...
	void x_Graphics::present()
	{
		if (eglSwapBuffers(eglDisplay, eglSurface) != GL_TRUE)
		{
			...
			if (!x_Graphics::create(m_windowHandler, m_deviceContextHandler))
			{
	...
	void x_Graphics::size(x_Sint32 width, x_Sint32 height)
	{
